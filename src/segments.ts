 ...
 */
export function splitOversized (units: Unit[], maxChars = MAX_UNIT_CHARS): Unit[] {
  const out: Unit[] = []
  for (const unit of units) {
    if (unit.text.length <= maxChars) { out.push(unit); continue }
    const parts: string[] = []
    let chunk: string[] = []
    let size = 0
    const flush = (): void => {
    ...
    for (const line of unit.text.split('\n')) {
      if (line.length > maxChars) {
        flush()
        for (let at = 0; at < line.length; at += maxChars) parts.push(line.slice(at, at + maxChars))
        continue
      }
 ...
 * just what it says.
 */
export function diffUnits (diff: string, runLines = 12, max = 40): Unit[] {
 ...
 */
export function textUnits (text: string, max = 40, minChars = 24): Unit[] {
...
export function unitsOf (text: string, max = 40): Unit[] {
  /*
 ...
 */
export function hunksOf (diff: string, max = 40, maxLines = 60): Unit[] {
  const lines = diff.split('\n')
  const units: Unit[] = []
  let header = ''
  let body: string[] = []
  ...
  flush()
  if (units.length) return splitOversized(units.slice(0, max))
