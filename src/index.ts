...
export function apply (ctx: KitContext, input: Partial<Config> = {}): void {
  ...
  const keyFingerprint = (value: string): string => createHash('sha256').update(value).digest('hex').slice(0, 12)
  ...
  /*
   * A second limiter, not a shared one: sharing means a foreground judgment queues
   * behind whatever batch is in flight, so an advisory instrument would add latency to
   * the turn it is supposed to help. Rebuilt whenever settings change, with the rest.
   */
  let fgLimiter = createLimiter(config.foregroundConcurrency)
  ...
  function primaryEngine (): { engine: Engine, unknown: string[] } {
    const { engines, unknown } = selectEngines(config.engines.length ? config.engines : ['jev'], knownEngines())
    return { engine: engines[0] ?? knownEngines()[0] as Engine, unknown }
  }

  /**
   * The engine that serves one channel: a per-channel route if one is configured.
   *
   * Falls back to the primary engine when the route names an engine this build does
   * not know, rather than failing the judgment — a typo in a knob must not silently
   * disable a channel, and `status` reports the unknown id.
   */
  function engineFor (channelId: string): { engine: Engine, unknown: string[] } {
    const wanted = config.engineByChannel?.[channelId]
    if (!wanted) return primaryEngine()
  ...
  async function judge (channel: ChannelSpec, channelState: ChannelState, session: string, failures?: string[], lane: 'foreground' | 'background' = 'background'): Promise<ItemResult | null> {
    /**
     * Record why a unit produced no verdict.
     *
     * A count was not enough: the shell-reachable gates (`scan-private`, `check-commit`)
     * report `flagged`, and a unit that was never judged is not a unit that was found
     * clean — but with only a counter, "the model never answered" and "nothing here"
     * left the route looking identical (`ok:true, flagged:0`), so a pre-push gate could
     * pass on exactly the biggest diffs. The reason travels with it so the gate can say
     * which it was, and so the cause is not guessed at by the renderer.
     */
    const fail = (why: string): null => { failures?.push(`${channel.id}: ${why}`); return null }
    ...
    const { engine } = engineFor(channel.id)
    /*
  ...
  function renderCall (result: CallResult, note?: string): string {
    ...
    const warns = [
      ...
      /*
       * The reasons, not a roster of guesses. This line used to hardcode
       * "key/熔断/预算", so a unit the model refused for being too large was reported
       * as a credential problem — the wrong cause, in the one line whose whole job is
       * to say "this was not a clean answer".
       */
      result.degraded
        ? `（${result.degraded} 条**未能判定**，已 fail-open —— 这不等于"没问题"${result.errors.length ? `：${[...new Set(result.errors)].slice(0, 3).join('；')}` : ''}）`
        : '',
      result.stopReason ?? '',
    ].filter(Boolean)
  ...
  function statusPayload (): Record<string, unknown> {
    return {
      ...
      settings: {
        ...
        /*
         * The foreground lane and the routing table are in the payload because a knob
         * nobody can see is a knob nobody can trust: "is the critical path still 1.2s"
         * and "which channels go to the local engine" are exactly the questions a reader
         * asks of an instrument that runs alongside their turns.
         */
      ...
      lanes: { background: { inFlight: limiter.active, queued: limiter.queued }, foreground: { inFlight: fgLimiter.active, queued: fgLimiter.queued } },
      ...
      health: {
        ...
        foregroundInFlight: fgLimiter.active,
        foregroundQueued: fgLimiter.queued,
...
/* ── tools ──────────────────────────────────────────────────────────── */
  ...
  /* ── the automatic lane ──────────────────────────────────────────────
   *
   * The structural gap this closes: kit was purely on-demand (tools, git hooks,
   * routes), so a channel only fired if a human or a hook remembered to ask. Every
   * channel built to *replace* a frontier round trip — `sufficient`, `route`,
   * `duplicate_call`, `evidence_check` — therefore sat at **zero** calls, and the
   * family's conclusion was that the bottleneck is entry points, not channels. Lens
   * has had a `pre`/`post-execute` lane all along; this is kit's.
   *
   * Deliberately narrow: one observer, one situation (a tool call that failed), one
   * channel, a per-session ceiling, and `shadow` by default so the lane's first job is
   * to prove it would have been right rather than to start talking during a turn.
   */

  /** Automatic judgments spent, per session. Bounded so the lane cannot outspend a turn. */
  const autoSpent = new Map<string, number>()

  const sessionOfCall = (exec: unknown): string =>
    (exec as { agent?: { session?: { id?: string } } })?.agent?.session?.id ?? 'unknown'

  /** The failure text a `failure_triage` judgment needs, from whatever shape the result has. */
  function failureTextOf (result: unknown): string {
    const fromContexts = (result as { additionalContexts?: Array<{ content?: Array<{ text?: string }> }> })?.additionalContexts
      ?.flatMap(context => (context.content ?? []).map(part => part.text ?? '')).join('\n')
    const visible = (result as { content?: Array<{ text?: string }> })?.content
      ?.map(part => part.text ?? '').join('\n')
    return (fromContexts || visible || '').trim().slice(0, 20_000)
  }

  function installAutomaticLane (host: KitContext): void {
    const on = host.on
    if (typeof on !== 'function') return
    const subscribe = on.bind(host)
    host.effect(() => subscribe('tools/post-execute', async (exec: never, result: never, next: () => Promise<unknown>) => {
      /*
       * `next()` first, always: this lane observes, it never gates. Whatever the tool
       * was going to return, it still returns that, unchanged.
       */
      const decision = await next()
      try {
        const channel = config.autoChannels[0] ?? ''
        const session = sessionOfCall(exec)
        const plan = autoPlan({
          mode: config.autoTriage,
          autoChannels: config.autoChannels,
          channel,
          isError: (result as { isError?: boolean })?.isError === true,
          usedThisSession: autoSpent.get(session) ?? 0,
          cap: config.autoMaxPerSession,
        })
        if (plan === 'skip') return decision
        const text = failureTextOf(result)
        if (!text) return decision
        autoSpent.set(session, (autoSpent.get(session) ?? 0) + 1)
        /*
         * Fire and forget: in shadow mode nothing waits on this, so a slow judgment
         * costs a missing ledger row rather than a stalled turn. It lands in the ledger
         * under the `auto` entry, which is how the report answers "would the lane have
         * had something to say".
         */
        void runInput(channel, { text, task: (exec as { name?: string })?.name ?? '' }, 'auto').catch(() => {})
      } catch { /* a measurement must never break dispatch */ }
      return decision
    }), 'jev-kit post-execute observer (automatic lane)')
  }
          ...
          handler: async (req: WebRequestLike, res: WebResponseLike) => {
            ...
            try {
              ...
              if (req.method === 'POST' && route === 'config') {
                ...
                fgLimiter = createLimiter(config.foregroundConcurrency)
              ...
              if (req.method === 'POST' && route === 'i18n-check') {
                 ...
                 */
                const body = await readBody(req) as { newKeys?: Array<{ key: string, text: string }>, existing?: Array<{ key: string, text: string }> }
                ...
                if (!incoming.length) { send(res, 200, { ok: true, checked: 0, flagged: 0, findings: [], markdown: '没有新增词条。' }); return }
                const units: Unit[] = incoming.map(entry => ({ text: `${entry.key}：${entry.text}`, where: entry.key }))
              ...
              if (req.method === 'POST' && route === 'scope-check') {
                 ...
                 */
                const body = await readBody(req) as { task?: string, diff?: string }
                const task = typeof body?.task === 'string' ? body.task.trim() : ''
                const diff = typeof body?.diff === 'string' ? body.diff : ''
                if (!task || !diff) { send(res, 400, { ok: false, error: '需要 task 与 diff' }); return }
                const units = hunksOf(diff, config.maxItems)
                if (!units.length) { send(res, 200, { ok: true, units: 0, flagged: 0, findings: [], markdown: 'diff 里没有可判定的 hunk。' }); return }
                ...
                send(res, 200, { ok: true, units: result.items.length, flagged: findings.length, failed: result.degraded, failures: result.errors, findings, markdown: renderCall(result) })
                return
              }
              if (req.method === 'POST' && route === 'check-commit') {
                 ...
                 */
                const body = await readBody(req) as { message?: string, diff?: string }
              ...
              if (req.method === 'POST' && route === 'key') {
                ...
                if (!value) { send(res, 400, { ok: false, error: '空 key' }); return }
                const service = services.credentials
              ...
              if (req.method === 'POST' && route === 'key/clear') {
                const service = services.credentials
                if (service && typeof service.unset === 'function') await service.unset(config.apiKeyRef)
                keyState.value = ''
                await ensureJev()
              ...
              if (req.method === 'POST' && route === 'scan-private') {
                 ...
                 */
                const body = await readBody(req) as { diff?: string, text?: string, maxItems?: number }
                const artifact = typeof body?.diff === 'string' && body.diff ? body.diff : (typeof body?.text === 'string' ? body.text : '')
                if (!artifact) { send(res, 400, { ok: false, error: '需要 diff 或 text' }); return }
                const cap = typeof body?.maxItems === 'number' ? Math.max(1, Math.min(200, body.maxItems)) : config.maxItems
                const units = unitsOf(artifact, cap)
                if (!units.length) { send(res, 200, { ok: true, units: 0, flagged: 0, findings: [], markdown: '没有可扫描的内容（diff 里没有新增行）。' }); return }
                ...
                send(res, 200, {
                  ...
                  flagged: findings.length,
                  /*
                   * `failed` is the field a gate needs and did not have: these units were
                   * never judged, so `flagged: 0` beside them means "no verdict", not
                   * "clean". A hook that reads only `flagged` treats the two the same.
                   */
