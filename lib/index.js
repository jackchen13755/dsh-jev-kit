...
import { MAX_UNIT_CHARS, hunksOf, unitsOf } from './segments.js';
import { jevEngine, layaEngine, selectEngines, trimState } from './engines.js';
import { FIXTURES, check, questionsFor, renderBench, summarizeEngine, verdictFor, fittedTable, fitVerdictOf, setThresholdOverrides, thresholdOverrides, thresholdOf, questionHash, questionHashes, wordingDrift } from './bench.js';
import { append, load, render, summarize } from './ledger.js';
import { KIT_DEFAULTS, autoPlan, loadStored, merge, saveStored, validate } from './settings.js';
...
export function apply(ctx, input = {}) {
    ...
    async function judge(channel, channelState, session, failures, lane = 'background') {
        /**
         ...
         */
        const fail = (why) => { failures?.push(`${channel.id}: ${why}`); return null; };
    ...
    async function runUnits(channelId, units, session, extra = {}, stateOf) {
        ...
        const workers = Array.from({ length: Math.min(config.concurrency, capped.length) }, async () => {
            while (cursor < capped.length) {
                 ...
                 */
                const channelState = stateOf ? stateOf(unit) : { text: unit.text, ...extra };
    ...
    const autoSpent = new Map();
    const sessionOfCall = (exec) => exec?.agent?.session?.id ?? 'unknown';
    /** The failure text a `failure_triage` judgment needs, from whatever shape the result has. */
    function failureTextOf(result) {
        const fromContexts = result?.additionalContexts
            ?.flatMap(context => (context.content ?? []).map(part => part.text ?? '')).join('\n');
        const visible = result?.content
            ?.map(part => part.text ?? '').join('\n');
        return (fromContexts || visible || '').trim().slice(0, 20000);
    }
    function installAutomaticLane(host) {
        const on = host.on;
        if (typeof on !== 'function')
            return;
        const subscribe = on.bind(host);
        host.effect(() => subscribe('tools/post-execute', async (exec, result, next) => {
            /*
            ...
            try {
                 ...
                 */
                void runInput(channel, { text, task: exec?.name ?? '' }, 'auto').catch(() => { });
            }
                    ...
                    handler: async (req, res) => {
                        ...
                        try {
                            ...
                            if (req.method === 'POST' && route === 'triage') {
                                ...
                                if (!channel) {
                                    send(res, 400, { ok: false, error: `未知通道：${JSON.stringify(body?.channel)}` });
                                    return;
                                }
                                const session = typeof body?.context === 'string' && body.context ? body.context : 'tool';
                                const strings = (value, cap) => Array.isArray(value) ? value.filter((x) => typeof x === 'string' && x.trim().length > 0).slice(0, cap) : undefined;
                                if (channel.per === 'input') {
                                    ...
                                    const text = typeof body?.text === 'string' && body.text.trim()
                                        ? body.text
                                        : (Array.isArray(body?.items) ? body.items.filter((x) => typeof x === 'string').join('\n') : '');
                                    /*
                                ...
                                const items = (Array.isArray(body?.items) ? body.items : []).filter((x) => typeof x === 'string' && x.trim().length > 0).slice(0, config.maxItems);
                                if (!items.length) {
                                    send(res, 400, { ok: false, error: 'items 为空（逐条判定通道需要 items）' });
                                    return;
                                }
                            ...
                            if (req.method === 'POST' && route === 'acted') {
                                ...
                                if (!channel) {
                                    send(res, 400, { ok: false, error: '需要已知的 channel' });
                                    return;
                                }
                                append(ledgerDir, {
                                    t: Date.now(),
            ...
            scope.effect(() => scope.commands?.register({
                ...
                handler: async (invocation) => {
                    ...
                    if (sub === 'triage') {
                        ...
                        if (!target)
                            return { kind: 'error', text: '用法：/jev-kit triage <日志文件> [通道]（通道默认 log_triage；`-` 表示读标准输入）' };
                        let text = '';
                        try {
                            text = target === '-' ? '' : fs.readFileSync(target, 'utf8');
                        }
                        catch (error) {
                            return { kind: 'error', text: `读不到 ${target}：${error.message}` };
                        }
                        if (!text.trim())
                            return { kind: 'error', text: `${target} 是空的（或用了 '-' 但本命令没有 stdin）。` };
                        ...
                        return { kind: 'success', text: `**${target}**（${lines.length} 行 → ${result.items.length} 段）\n\n${renderCall(result)}` };
                    }
                    if (sub === 'report') {
