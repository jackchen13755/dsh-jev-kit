    ...
    for (const { axis, pool } of axes) {
        for (let i = 0; i < 10; i++) {
            ...
            out.push({
                ...
                state: { text: SCENES[i % SCENES.length](body) },
            });
        }
    }
...
function logFixtures() {
    ...
    return rows.flatMap(([label, lines, expected]) => lines.map((line, index) => ({
        ...
        expect: { kind: 'choice', field: 'kind', equals: expected },
        state: { text: line },
    })));
}
/* ── A/B/C/D channels ────────────────────────────────────────────────── */
const paired = (channel, field, entries, high, low) => entries.map(([text, isHigh, truth], index) => ({
    id: `${channel}_${isHigh ? 'hi' : 'lo'}_${index}`,
    channel,
    truth,
    expect: { kind: isHigh ? 'high' : 'low', field },
    state: { text, task: high, other: low },
}));
function judgementFixtures() {
...
function pickFixtures() {
    return pickCases.map(item => ({
        ...
        channel: 'pick',
        truth: item.truth,
        expect: { kind: 'choice', field: 'choice', equals: item.right < 0 ? 'none_of_these' : `c${item.right}` },
...
function pageFixtures() {
    return PAGE_CASES.map(item => ({
        ...
        channel: 'page_state',
        truth: item.truth,
        expect: { kind: 'choice', field: 'state', equals: item.state },
...
export function corpusByChannel(fixtures = CORPUS) {
    const counts = {};
    for (const fixture of fixtures)
        counts[fixture.channel] = (counts[fixture.channel] ?? 0) + 1;
    return counts;
}
