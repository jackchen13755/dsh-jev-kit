 ...
 */
export const fitVerdictOf = (fit) => {
    if ((fit.separation ?? 0) < FITTABLE_SEPARATION)
        return 'noisy';
...
export const FIT_VERDICT_MARK = { adjustable: '🟢', optimal: '⚪', noisy: '⬛' };
/*
...
export function fitThresholds(fixtures, trials, current = DEFAULT_THRESHOLDS) {
    ...
    for (const channel of [...new Set(fixtures.map(fixture => fixture.channel))]) {
        ...
        const recommendedBy = recommended === marginCut ? 'margin' : 'accuracy';
