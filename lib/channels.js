...
export const CHANNELS = {
    ...
    private_scan: {
        ...
        questions: state => ({
            ...
            internal: noul('Does `text` reveal an organisation-internal detail that should not be public — an intranet hostname, an internal repository or service name, or an internal project codename?', 'an internal hostname, service, or codename is exposed', 'nothing that identifies an internal system'),
        }),
        read: (answers, state) => {
    ...
    scope_check: {
        ...
        at: 0.5,
        questions: () => ({
            in_scope: noul('Is the change in `hunk` required by `task` — either to satisfy it directly or to fix something it necessarily breaks?', 'the change is required by the task', 'the change is unrelated to the task, or is a drive-by improvement the task did not ask for'),
            necessary: noul('Would `task` still be satisfied if `hunk` were reverted and nothing else changed?', 'reverting this change would leave the task unsatisfied', 'reverting this change would still leave the task satisfied, so it is optional'),
        }),
        read: (answers, state) => {
            const inScope = p(answers, 'in_scope');
            const necessary = p(answers, 'necessary');
            const out = inScope !== undefined && inScope < 0.5;
            const optional = !out && necessary !== undefined && necessary < 0.5;
            return {
                level: out ? 'flag' : optional ? 'warn' : 'info',
                headline: out ? '⚠️ 疑似超出范围' : optional ? '△ 可能非必需' : '在范围内',
                details: [`in_scope=${show(inScope)} necessary=${show(necessary)}`, `改动：${(state.text ?? '').slice(0, 100)}`],
    ...
    memory_conflict: {
        ...
        at: 0.6,
        questions: () => ({
            contradicts: noul('Do `a` and `b` make conflicting claims — following one would mean violating the other?', 'they conflict: both cannot be followed at once', 'they are compatible; they may overlap but do not conflict'),
            duplicate: noul('Do `a` and `b` state the same fact, such that one of them is redundant?', 'the same fact stated twice', 'different facts, even if they are about the same topic'),
        }),
        read: answers => {
    ...
    sufficient: {
        ...
        at: 0.8,
        questions: () => ({
            answers: noul('Do `results` already contain everything `task` asks for, so that a final answer could be written right now without another tool call?', 'every part of the request is already covered by the results', 'at least one part of the request still needs another tool call'),
            missing: noul('Is there a specific piece of information that `task` requires and `results` clearly lack?', 'a required piece is missing', 'nothing required is missing'),
        }),
        read: answers => {
    ...
    duplicate_call: {
        ...
        at: 0.7,
        questions: () => ({
            equivalent: noul('Is `call` equivalent to one of `prior` — same target and same intent, such that its result would already be known?', 'yes: an earlier call already covers it', 'no: it asks for something the earlier calls did not'),
        }),
        read: answers => {
    ...
    evidence_check: {
        ...
        questions: state => {
            ...
            requirements.forEach((requirement, index) => {
                out[`r${index}`] = noul(`Does \`report\` contain evidence that satisfies this specific requirement: "${requirement.slice(0, 200)}"?`, 'the report shows concrete evidence for it', 'the report does not show evidence for it, or only asserts it');
            });
            return out;
        },
        read: (answers, state) => {
    ...
    risk: {
        ...
        at: 0.6,
        questions: () => ({
            irreversible: noul('Would `action` cause an effect that cannot be undone — deleting data that is not reproducible, overwriting a remote, publishing, paying, or messaging a third party?', 'the effect cannot be undone by re-running or restoring', 'the effect is local and reversible, or touches only reproducible artifacts'),
            external: noul('Does `action` affect anything outside this machine and this repository?', 'it reaches a remote system, a shared service, or another person', 'it stays on this machine and in this working tree'),
        }),
        read: answers => {
    ...
    commit_message: {
        ...
        at: 0.5,
        questions: () => ({
            matches: noul('Does `message` describe what `diff` actually changes?', 'the message describes the change accurately', 'the message describes something else, or omits the main change'),
            unrelated: noul('Does `diff` contain changes unrelated to what `message` describes?', 'yes: there are changes the message does not account for', 'no: every change in the diff is accounted for by the message'),
        }),
        read: answers => {
    ...
    plan_risk: {
        ...
        at: 0.6,
        questions: () => ({
            needs_decision: noul('Does `step` require a human decision that the agent cannot make on its own — a product tradeoff, a destructive choice, or spending money?', 'a human must choose before this can proceed', 'the step follows from the request; no new decision is needed'),
            external: noul('Does `step` publish, deploy, message someone, or otherwise leave this machine irreversibly?', 'yes: it has an irreversible external effect', 'no: it is local and reversible'),
        }),
        read: answers => {
    ...
    log_triage: {
        ...
        questions: () => ({
            ...
            actionable: noul('Does `line` point at something the reader should fix or investigate?', 'yes: it names a problem to act on', 'no: it is informational'),
        }),
        read: answers => {
            const kind = pickOf(answers, 'kind');
            return {
    ...
    alert_dedup: {
        ...
        at: 0.6,
        questions: () => ({
            same: noul('Is `alert` the same incident as `open` — the same underlying cause at the same place?', 'the same incident, described again', 'a different incident, even if the symptoms look similar'),
        }),
        read: answers => {
    ...
    bug_triage: {
        ...
        questions: () => ({
            ...
            reproducible: noul('Does `report` contain enough steps or evidence for someone to reproduce the problem?', 'yes: the steps are specific enough to follow', 'no: it describes a symptom without a way to reproduce it'),
        }),
        read: answers => {
            const kind = pickOf(answers, 'kind');
    ...
    flaky: {
        ...
        at: 0.6,
        questions: () => ({
            flaky: noul('Does `failure` look non-deterministic — a timeout, a race, an ordering dependency, or a resource contention — rather than a deterministic assertion about the change?', 'yes: the shape points at non-determinism', 'no: it reads as a deterministic failure caused by the change'),
            assertion_related: noul('Does the failure name an assertion or behaviour that the recent change could plausibly have altered?', 'yes: it is about something the change touched', 'no: it is unrelated to what changed'),
        }),
        read: answers => {
