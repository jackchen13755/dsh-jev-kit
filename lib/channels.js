...
export const CHANNELS = {
    ...
    private_scan: {
        ...
        questions: state => ({
            ...
            personal: noul('Does `text` contain personal or identifying information about a real person — a home directory path, a personal email address, a phone number, a full name tied to an account, or an internal username?', 'personal or identifying information about a real person is present', 'no personal information; generic placeholders, and identity a project publishes on purpose (its own author/maintainer name, GitHub handle or users.noreply.github.com address in a manifest) do not count'),
            /*
             ...
             */
            internal: noul('Does `text` reveal an organisation-internal detail that should not be public — an intranet hostname, an internal repository or service name, or an internal project codename?', 'an intranet hostname, an internal-only service or repository, or an internal codename is exposed', 'nothing that identifies an internal system; ordinary source code, UI copy and configuration keys of the repository being scanned are not internal details, and neither are public package names, a public vendor\'s documented API endpoint, localhost addresses or open-source project names'),
        }),
        read: (answers, state) => {
             ...
             */
            const cutOf = (axis) => readerThreshold(`private_scan.${axis}`, readerThreshold('private_scan', 0.5));
            const findings = [];
            if ((secret ?? 0) >= cutOf('secret'))
                findings.push('凭据/密钥');
            if ((personal ?? 0) >= cutOf('personal'))
                findings.push('个人信息');
            if ((internal ?? 0) >= cutOf('internal'))
                findings.push('内网信息');
            return {
    ...
    page_state: {
        ...
        read: answers => {
            ...
            const chosen = move[state] ?? move.unknown;
            const labels = {
                target: '目标页（内容与控制都在）',
                shell: '空壳（框架在、数据未到）',
                login: '登录页/会话过期',
